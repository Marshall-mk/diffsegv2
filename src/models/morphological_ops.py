...
class PolynomialMorphology(nn.Module):
    ...
    def polynomial_dilation(self, x: torch.Tensor) -> torch.Tensor:
        ...
        kernel_mask = self.kernel.view(1, 1, -1, 1, 1).to(neighborhoods.device)
        complement = complement * kernel_mask + (
            1 - kernel_mask
        )  # Set masked areas to 1
...
class MorphologicalDegradation(nn.Module):
    ...
    def _create_morphological_schedule(self):
        ...
        if self.morph_routine == "Progressive":
            ...
            kernel_sizes = np.linspace(
                self.kernel_size_start, self.kernel_size_end, self.num_timesteps
            )
            kernel_sizes = [
                int(k) if int(k) % 2 == 1 else int(k) + 1 for k in kernel_sizes
            ]  # Ensure odd

            for i, kernel_size in enumerate(kernel_sizes):
                morph_op = PolynomialMorphology(
                    kernel_size=kernel_size, channels=self.channels
                )
            ...
            for i in range(self.num_timesteps):
                morph_op = PolynomialMorphology(
                    kernel_size=self.kernel_size_start, channels=self.channels
                )
                if self.device_of_kernel == "cuda":
                    morph_op = morph_op.cuda()
                operators.append(morph_op)
...
def create_morphological_schedule(
    timesteps: int, schedule_type: str = "exponential"
) -> torch.Tensor:
...
def apply_morphological_degradation(
    mask: torch.Tensor,
    intensity: float,
    morph_type: str = "dilation",
    soft_morph: PolynomialMorphology = None,
) -> torch.Tensor:
    """
    ...
    if soft_morph is None:
        soft_morph = PolynomialMorphology(
            kernel_size=3, channels=mask.shape[1] if len(mask.shape) > 1 else 1
        )
        if mask.is_cuda:
    ...
    if torch.isnan(result).any():
        print(
            "Warning: NaN values detected in morphological degradation, returning original mask"
        )
        return mask.clone()
...
class MorphologicalDebugger:
    ...
    def check_gradient_flow(model, sample_input, timesteps_to_check=[0, 10, 25, 49]):
        ...
        for t in timesteps_to_check:
            ...
            print(
                f"Timestep {t}: Loss = {loss.item():.6f}, Gradient norm = {grad_norm:.6f}"
            )
    ...
    def compare_morphological_operations(mask, kernel_sizes=[3, 5, 7]):
        ...
        for kernel_size in kernel_sizes:
            morph_op = PolynomialMorphology(
                kernel_size=kernel_size, channels=mask.shape[1]
            )
            if mask.is_cuda:
                morph_op = morph_op.cuda()
