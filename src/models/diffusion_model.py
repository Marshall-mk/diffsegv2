...
class DiffusionSegmentation(nn.Module):
    def __init__(
        ...
        morph_type: str = "mixed",
        morph_kernel_size_start: int = 3,
        morph_kernel_size_end: int = 9,
        morph_schedule_type: str = "exponential",
        morph_routine: str = "Progressive",
        ...
        if unet_type == "diffusers_2d":
            ...
            raise ValueError(
                f"Unsupported unet_type: {unet_type}. Choose from: 'diffusers_2d', 'diffusers_2d_cond'"
            )
        ...
        if diffusion_type == "gaussian":
            ...
            if scheduler_type == "ddpm":
                ...
                raise ValueError(
                    f"Unsupported scheduler_type: {scheduler_type}. Choose from: 'ddpm', 'ddim'"
                )
            ...
            self.register_buffer(
                "morph_schedule",
                create_morphological_schedule(timesteps, morph_schedule_type),
            )
            ...
            raise ValueError(
                f"Unsupported diffusion_type: {diffusion_type}. Choose from: 'gaussian', 'morphological'"
            )
    ...
    def forward_morphology(self, x0: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(batch_size):
            ...
            for step in range(timestep + 1):
                ...
                current_mask = self.morph_degradation.forward(
                    current_mask, step, intensity
                )
    ...
    def forward_morphology_batch(
        ...
        for timestep in unique_timesteps:
            ...
            for step in range(timestep.item() + 1):
                ...
                current_batch = self.morph_degradation.forward(
                    current_batch, step, intensity
                )
    ...
    def forward(
        self, image: torch.Tensor, mask: torch.Tensor = None, t: torch.Tensor = None
    ) -> torch.Tensor:
        if self.training:
            if mask is None:
            ...
            if t is None:
                t = torch.randint(
                    0, self.timesteps, (image.shape[0],), device=image.device
                )
    ...
    def sample(
        ...
        if self.diffusion_type == "gaussian":
            ...
            for t in self.scheduler.timesteps:
                t_tensor = torch.full(
                    (image.shape[0],), t, device=image.device, dtype=torch.long
                )
            ...
            for i, current_t in enumerate(timesteps):
                t_tensor = torch.full(
                    (image.shape[0],), current_t, device=image.device, dtype=torch.long
                )
    ...
    def compute_loss(
        ...
        target: torch.Tensor,
        loss_type: str = "mse",
        use_morphological_loss: bool = True,
    ) -> torch.Tensor:
        """
        ...
        if predicted.shape[-1] > 1 and predicted.shape[-2] > 1:
            ...
            boundary_loss = F.mse_loss(pred_grad_x, target_grad_x) + F.mse_loss(
    ...
    def get_batch_training_info(
        self, image: torch.Tensor, mask: torch.Tensor, t: torch.Tensor = None
    ) -> dict:
        """
        ...
        if self.diffusion_type == "gaussian":
            ...
            intensities = [
                self.morph_schedule[timestep.item()].item() for timestep in t
            ]
    ...
    def visualize_training_flow(
        self, image: torch.Tensor, mask: torch.Tensor, num_timesteps_to_show: int = 6
    ) -> dict:
        """
        ...
        timestep_indices = torch.linspace(
            0, self.timesteps - 1, num_timesteps_to_show
        ).long()
