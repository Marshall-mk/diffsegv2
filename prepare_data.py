...
def analyze_dataset(image_dir: str, mask_dir: str) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "mask_sizes": {
            ...
            "most_common_size": max(set(mask_sizes), key=mask_sizes.count)
            if mask_sizes
            else None,
        },
    }
...
def validate_dataset(
    image_dir: str, mask_dir: str, fix_issues: bool = False
) -> List[str]:
    ...
    try:
        ...
        for i in range(min(5, len(dataset))):
            try:
                ...
                if image.shape[0] != 3:
                    issues.append(
                        f"Image {info['image_name']} has {image.shape[0]} channels, expected 3"
                    )

                if mask.shape[0] != 1:
                    issues.append(
                        f"Mask {info['mask_name']} has {mask.shape[0]} channels, expected 1"
                    )

                # Check value ranges
                if image.min() < -1.1 or image.max() > 1.1:
                    issues.append(
                        f"Image {info['image_name']} values out of expected range [-1, 1]"
                    )

                if mask.min() < -0.1 or mask.max() > 1.1:
                    issues.append(
                        f"Mask {info['mask_name']} values out of expected range [0, 1]"
                    )
...
def organize_dataset(source_dir: str, output_dir: str, train_split: float = 0.8):
    ...
    print(
        f"Organizing {len(train_files)} training files and {len(val_files)} validation files..."
    )

    # Copy files
    for file_list, img_dir, mask_dir in [
        (train_files, train_images_dir, train_masks_dir),
        ...
        for file_path in tqdm(file_list):
            ...
            if any(
                ...
                for keyword in ["mask", "gt", "label", "annotation"]
            ):
                dest_dir = mask_dir
            else:
                dest_dir = img_dir
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare data for Diffusion Segmentation"
    )
    parser.add_argument(
        "--action",
        type=str,
    ...
    parser.add_argument(
        "--source-dir", type=str, help="Source directory for organization"
    )
    ...
    parser.add_argument(
        "--num-samples", type=int, default=100, help="Number of samples to create"
    )
    parser.add_argument(
    ...
    if args.action == "analyze":
        ...
        print(
            f"Image width range: {stats['image_sizes']['min_width']} - {stats['image_sizes']['max_width']}"
        )
        print(
            f"Image height range: {stats['image_sizes']['min_height']} - {stats['image_sizes']['max_height']}"
        )
