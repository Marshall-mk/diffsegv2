...
def load_model(
    ...
    model_params = {
        ...
        "pretrained_model_name_or_path": saved_config.get(
    ...
    if saved_config.get("diffusion_type", diffusion_type) == "morphological":
        ...
        model_params.update(
            {
                "morph_kernel_size_start": saved_config.get(
                    "morph_kernel_size_start", morph_kernel_size_start
                ),
                "morph_kernel_size_end": saved_config.get(
                    "morph_kernel_size_end", morph_kernel_size_end
                ),
                "morph_routine": saved_config.get("morph_routine", morph_routine),
                "morph_schedule_type": saved_config.get(
                    "morph_schedule", morph_schedule_type
                ),
            }
        )
        ...
        if morph_kernel_size is not None:
            warnings.warn(
                "morph_kernel_size is deprecated. Using as morph_kernel_size_start."
            )
            model_params["morph_kernel_size_start"] = morph_kernel_size

        # Legacy compatibility for old checkpoints
        if (
            "morph_kernel_size" in saved_config
            and "morph_kernel_size_start" not in saved_config
        ):
            warnings.warn(
                "Loading legacy checkpoint. Converting morph_kernel_size to new parameters."
            )
            model_params["morph_kernel_size_start"] = saved_config["morph_kernel_size"]
            model_params["morph_kernel_size_end"] = (
                saved_config["morph_kernel_size"] + 4
            )
...
def predict_single_image(
    ...
    with torch.no_grad():
        ...
        predicted_mask = model.sample(
            image, mask=None, num_inference_steps=num_inference_steps
        )

    return predicted_mask


def predict_with_intermediate_steps(
    model: DiffusionSegmentation,
    image_path: str,
    device: torch.device,
    num_inference_steps: int = 50,
    ...
    with torch.no_grad():
        for i, current_t in enumerate(tqdm(timesteps, desc="Inference steps")):
            t_tensor = torch.full(
                (image.shape[0],), current_t, device=device, dtype=torch.long
            )
...
def batch_inference(
    ...
    for image_file in tqdm(image_files, desc="Processing images"):
        try:
            ...
            if save_intermediate and model.diffusion_type == "morphological":
                ...
                if intermediate_steps:
                    ...
                    for step_data in intermediate_steps:
                        ...
                        step_file = (
                            step_dir
                            / f"step_{step_data['step']:03d}_t{step_data['timestep']:04d}.png"
                        )
            ...
            results_summary.append({"image": image_file.name, "error": str(e)})
...
def interactive_inference(
    ...
    while True:
        command = input(
            f"\n[{current_steps} steps, threshold {current_threshold:.2f}] > "
        ).strip()
        ...
        if command.startswith("debug "):
            ...
            try:
                ...
                for step_data in intermediate_steps:
                    print(
                        f"  Step {step_data['step']}: timestep {step_data['timestep']}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run inference with Enhanced Diffusion Segmentation Model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", type=str, default="./inference_results", help="Output directory"
    )
    parser.add_argument(
        "--batch", action="store_true", help="Batch inference mode (process directory)"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="Interactive inference mode"
    )
    parser.add_argument(
    ...
    # Legacy parameters
    parser.add_argument(
        "--morph-kernel-size", type=int, help="DEPRECATED: Use morph-kernel-size-start"
    )
    ...
    parser.add_argument(
        "--no-vis", action="store_true", help="Skip saving visualizations in batch mode"
    )
    parser.add_argument(
        "--mask-threshold",
        type=float,
    ...
    if args.morph_kernel_size is not None:
        warnings.warn(
            "--morph-kernel-size is deprecated. Use --morph-kernel-size-start"
        )
    ...
    if args.interactive:
        ...
        interactive_inference(
            model, device, args.steps, image_size, args.mask_threshold
        )
        ...
        if args.visualize_process and model.diffusion_type == "morphological":
            ...
            predicted_mask = predict_single_image(
                model, args.input, device, args.steps, image_size, show_progress=True
            )
        ...
        if (
            args.visualize_process
            and hasattr(model, "diffusion_type")
            ...
            try:
                plot_inference_steps(
                    model, image.to(device), num_steps=10, save_path=str(process_file)
                )
