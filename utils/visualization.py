...
def visualize_segmentation_legacy(
    image: torch.Tensor,
    mask: torch.Tensor,
    predicted_mask: Optional[torch.Tensor] = None,
    title: str = "Segmentation Results",
...
def visualize_diffusion_process(
    model,
    image: torch.Tensor,
    timesteps: List[int] = [0, 100, 500, 900, 999],
    save_path: Optional[str] = None,
):
...
def plot_inference_steps(
    model, image: torch.Tensor, num_steps: int = 10, save_path: Optional[str] = None
):
...
def visualize_training_flow(
    ...
    for i, (timestep, degraded_masks) in enumerate(
        zip(timesteps, training_data["degraded_masks"])
    ):
        if i >= max_timesteps:
            break
...
def visualize_batch_training_info(
    batch_info: Dict, save_path: str = None, max_samples: int = 4
):
    """
    ...
    plt.suptitle(
        f"Batch Training Info - {batch_info['diffusion_type'].title()} Diffusion",
        fontsize=16,
        fontweight="bold",
    )
    plt.tight_layout()
...
def create_training_flow_gif(
    ...
    try:
        ...
        print(
            "PIL and imageio required for GIF creation. Install with: pip install Pillow imageio"
        )
        return
