...
class SegmentationDataset(Dataset):
    ...
    def _find_files(self, extension: str) -> Tuple[List[Path], List[Path]]:
        ...
        image_files = sorted(
            [f for f in image_files if any(m.stem == f.stem for m in mask_files)]
        )
...
def create_synthetic_data(
    batch_size: int = 4, image_size: Tuple[int, int] = (256, 256)
    ...
    for i in range(batch_size):
        ...
        y, x = torch.meshgrid(
            torch.arange(image_size[0]), torch.arange(image_size[1]), indexing="ij"
        )
