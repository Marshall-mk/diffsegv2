...
def load_and_preprocess_image(image_path: str, size: Tuple[int, int] = (256, 256)) -> torch.Tensor:
...
def create_morphological_ops(morph_type: str, 
                           timesteps: int,
                           kernel_size_start: int,
                           kernel_size_end: int,
                           morph_routine: str,
                           schedule_type: str) -> MorphologicalDegradation:
...
def create_comparison_collage(results_dict: Dict[str, Dict[int, torch.Tensor]], 
                            timesteps_to_plot: List[int],
    ...
    gs = gridspec.GridSpec(n_morphs + 1, n_timesteps, height_ratios=[1] + [1] * n_morphs)
    ...
    for i, timestep in enumerate(timesteps_to_plot):
        ...
        if timestep == 0:
            ...
            ax.axis('off')
            ax.text(0.5, 0.5, f't={timestep}', ha='center', va='center', 
    ...
    for row, morph_type in enumerate(morph_types, 1):
        ...
        for col, timestep in enumerate(timesteps_to_plot):
            ...
            if timestep in results:
                ...
                # Add timestep label
                if col == 0:
                    ax.set_ylabel(f"{morph_type.title()}", fontsize=12, fontweight='bold')
                
                # Add intensity statistics
                mean_intensity = img_np.mean()
                ax.text(0.02, 0.02, f"μ={mean_intensity:.3f}", 
...
def create_individual_progression(results: Dict[int, torch.Tensor],
                                morph_type: str,
                                timesteps_to_plot: List[int],
                                config_info: str,
                                save_path: Optional[str] = None) -> None:
    ...
    for i, timestep in enumerate(timesteps_to_plot):
        ...
        if timestep in results:
            ...
            ax.set_title(f"t={timestep}\nμ={mean_intensity:.3f}, σ={std_intensity:.3f}\n"
                        f"[{min_intensity:.3f}, {max_intensity:.3f}]", fontsize=9)
        else:
...
def print_debug_info(morph_ops: MorphologicalDegradation, morph_type: str, schedule_type: str) -> str:
    ...
    if debug_info['kernel_sizes']:
        ...
        print(f"Sample Kernel Progression: {debug_info['kernel_sizes'][:5]}...{debug_info['kernel_sizes'][-5:]}")
    ...
    if hasattr(morph_ops, 'schedule'):
        ...
        print(f"Schedule Values: {schedule_values[:5].tolist()}...{schedule_values[-5:].tolist()}")
        print(f"Schedule Range: [{schedule_values.min().item():.6f}, {schedule_values.max().item():.6f}]")
    
    # Create configuration string for plots
    config_str = (f"Type: {debug_info['morph_type']}, "
...
def main():
    parser = argparse.ArgumentParser(description='Demo Morphological Forward Diffusion Processes')
    
    # Input parameters
    parser.add_argument('--input', type=str, required=True, 
                       help='Input image path')
    parser.add_argument('--output-dir', type=str, default='./morph_demo_results',
                       help='Output directory for results')
    
    # Diffusion parameters
    parser.add_argument('--timesteps', type=int, default=1000,
                       help='Total number of diffusion timesteps')
    parser.add_argument('--timesteps-to-plot', type=str, default='0,50,100,200,400,600,800,1000',
                       help='Comma-separated timesteps to visualize (e.g., "0,100,500,1000")')
    
    # Morphological parameters
    parser.add_argument('--morph-types', type=str, default='dilation,erosion,mixed,opening,closing',
                       help='Comma-separated morphological operation types to test')
    parser.add_argument('--kernel-size-start', type=int, default=3,
                       help='Starting kernel size')
    parser.add_argument('--kernel-size-end', type=int, default=9,
                       help='Ending kernel size')
    parser.add_argument('--morph-routine', type=str, default='Progressive',
                       choices=['Progressive', 'Constant'],
                       help='Morphological routine')
    parser.add_argument('--schedule-type', type=str, default='exponential',
    ...
    print(f"Image range: [{original_image.min().item():.3f}, {original_image.max().item():.3f}]")
    ...
    for morph_type in morph_types:
        ...
        try:
            # Create the morphological schedule first
            print(f"Creating {args.schedule_type} schedule for {args.timesteps} timesteps...")
            ...
            try:
                ...
                try:
                    schedule = create_morphological_schedule(
                        args.schedule_type,  # positional argument
                        args.timesteps
                    )
                except Exception as e2:
                    print(f"Error creating schedule with alternative parameters: {e2}")
                    print(f"Please check the create_morphological_schedule function signature")
                    raise e2
            ...
            print(f"Schedule created successfully. Sample values: {schedule[:5].tolist()}...{schedule[-5:].tolist()}")
            ...
            if args.debug_mode:
                ...
                config_strings[morph_type] = (f"Type: {morph_type}, "
            ...
            if args.create_individual_plots:
                ...
                create_individual_progression(
                    results, morph_type, timesteps_to_plot, 
                    config_strings[morph_type], str(individual_save_path)
                )
    ...
    if not args.no_collage and results_dict:
        ...
        config_summary = (f"Morphological Forward Diffusion Comparison\n"
                         f"Routine: {args.morph_routine}, "
                         f"Schedule: {args.schedule_type}, "
        ...
        create_comparison_collage(
            results_dict, timesteps_to_plot, original_image,
            config_summary, str(collage_save_path)
        )
    ...
    with open(config_file, 'w') as f:
        ...
        f.write(f"Kernel Size Range: {args.kernel_size_start} → {args.kernel_size_end}\n")
    ...
    print(f"- Individual plots: {'Created' if args.create_individual_plots else 'Skipped'}")
    print(f"- Comparison collage: {'Created' if not args.no_collage and results_dict else 'Skipped'}")
    ...
    if results_dict:
        print(f"\nNEXT STEPS:")
        print(f"1. Examine the comparison collage to see how different operations behave")
